...
AUTH_AWS_CODES = {"AccessDeniedException", "UnrecognizedClientException", "InvalidSignatureException", "ExpiredTokenException"}
...
# Pure helpers
# ---------------------------------------------------------------------------
def keyframe_interval(duration_ms: int, *, base_interval_sec: float, max_frames: int) -> float:
...
def resolve_thresholds(config: ApiCredentialConfig) -> dict:
    return {
        "block_threshold": float(config.get_option("block_threshold", getattr(settings, "FINAL_MODERATION_BLOCK_THRESHOLD", DEFAULT_BLOCK_THRESHOLD))),
        "flag_threshold": float(config.get_option("flag_threshold", getattr(settings, "FINAL_MODERATION_FLAG_THRESHOLD", DEFAULT_FLAG_THRESHOLD))),
...
def get_rekognition_config() -> ApiCredentialConfig:
    config = (
        ApiCredentialConfig.objects.filter(service=ServiceType.MODERATION, provider=REKOGNITION_PROVIDER, is_active=True, deleted_at__isnull=True)
...
class RekognitionModerator:
    ...
    def client(self):
        if self._client is None:
            ...
            kwargs = {"region_name": getattr(settings, "AWS_REKOGNITION_REGION", "us-east-1")}
            access_key = getattr(settings, "AWS_REKOGNITION_ACCESS_KEY_ID", "")
            if access_key:
                kwargs["aws_access_key_id"] = access_key
                kwargs["aws_secret_access_key"] = getattr(settings, "AWS_REKOGNITION_SECRET_ACCESS_KEY", "")
    ...
    def detect(self, image_bytes: bytes) -> FrameLabels:
        ...
        try:
            response = self.client.detect_moderation_labels(Image={"Bytes": image_bytes}, MinConfidence=self.min_confidence)
        except Exception as exc:  # botocore exceptions are mapped by name/code, no hard import
    ...
    def _map_exception(exc: Exception) -> Exception:
        ...
        if code in RETRYABLE_AWS_CODES or name in ("EndpointConnectionError", "ConnectionClosedError", "ReadTimeoutError", "ConnectTimeoutError"):
            if "Throttl" in code or "Exceeded" in code:
                return ProviderRateLimitError(f"Rekognition throttled the request ({code}).")
        ...
        if name == "NoCredentialsError" or name == "PartialCredentialsError":
            return ProviderNotConfigured("AWS Rekognition credentials are not configured (AWS_REKOGNITION_*).")
        return ProviderPermanentError(f"Rekognition rejected the request ({code or name}).", error_code="moderation_request_rejected")


# ---------------------------------------------------------------------------
# DB-aware orchestration
# ---------------------------------------------------------------------------
def extract_keyframes(video_path: str, out_dir: str, *, interval_sec: float, runner=media_tools.run_command) -> list[Path]:
    pattern = str(Path(out_dir) / "frame_%04d.jpg")
    runner(media_tools.build_keyframe_extract_command(video_path, pattern, interval_sec=interval_sec), timeout_sec=600)
    return sorted(Path(out_dir).glob("frame_*.jpg"))


def moderate_final_video_for_job(job, *, moderator=None, runner=media_tools.run_command, workdir: str | None = None) -> FinalModerationOutcome:
    ...
    if final_asset is None:
        raise JobNotReady(f"Video job {job.pk} has no final video — the assembly stage must run first.")
    ...
    interval = keyframe_interval(
        ...
        base_interval_sec=float(config.get_option("keyframe_interval_sec", getattr(settings, "FINAL_MODERATION_KEYFRAME_INTERVAL_SEC", DEFAULT_KEYFRAME_INTERVAL_SEC))),
        max_frames=int(config.get_option("max_frames", getattr(settings, "FINAL_MODERATION_MAX_FRAMES", DEFAULT_MAX_FRAMES))),
    )
    ...
    with tempfile.TemporaryDirectory(prefix=f"finalmod_{job.pk}_", dir=workdir) as tmp:
        ...
        frames = extract_keyframes(str(video_path), str(tmp_path), interval_sec=interval, runner=runner)
        if not frames:
            raise ProviderPermanentError("Keyframe extraction produced no frames.", error_code="keyframe_extraction_failed")
        ...
        for frame in frames:
            try:
                ...
                record_api_usage(
                    config=config, operation=OPERATION, job=job, user=job.user,
                    units=len(frame_results), unit_type="images", cost_usd=unit_cost(config, len(frame_results), expected_unit="per_request"),
                    http_status=getattr(exc, "http_status", None), success=False,
    ...
    record_api_usage(
        config=config, operation=OPERATION, job=job, user=job.user,
        units=len(frame_results), unit_type="images",
        cost_usd=unit_cost(config, len(frame_results), expected_unit="per_request"),
        latency_ms=total_latency, http_status=200, success=True,
    )
    refresh_job_cost(job)

    raw = {"frames": frame_results, "interval_sec": interval, "hits": hits, "min_confidence": moderator.min_confidence}
    ModerationLog.objects.create(
        ...
        verdict=verdict,
        categories=scores,
        threshold_config={**thresholds, "max_category": category, "max_score": score, "frames_checked": len(frame_results), "script_verdict_reused_for_audio": True},
        raw_response=raw,
    )
    logger.info(
        "final_moderation_completed",
        extra={"job_id": str(job.pk), "verdict": verdict, "max_category": category, "max_score": round(score, 4), "frames": len(frame_results)},
    )
    return FinalModerationOutcome(
        verdict=verdict, max_category=category, max_score=score, category_scores=scores,
        thresholds=thresholds, frames_checked=len(frame_results), raw_response=raw,
    )
...
def apply_final_verdict_to_job(job, outcome: FinalModerationOutcome) -> str:
    ...
    if outcome.verdict == ModerationVerdict.BLOCK and getattr(settings, "MODERATION_AUTO_REJECT_ON_BLOCK", False):
    ...
    try:
        ...
        notify_admins(
            "admin.alert",
            job=job,
            ctx={"subject": "Video needs moderator review", "message": f"Job {job.pk} ({job.title!r}) final moderation verdict={outcome.verdict}, category={outcome.max_category or 'n/a'}."},
            payload={"job_id": str(job.pk), "verdict": outcome.verdict, "stage": "final"},
        )
    except Exception:  # alerting must never undo the gate decision
        logger.exception("final_moderation_admin_alert_failed", extra={"job_id": str(job.pk)})
    return new_status
