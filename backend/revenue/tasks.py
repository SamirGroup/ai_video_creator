...
def close_revenue_period(period_start: str | None = None, period_end: str | None = None) -> dict:
    ...
    from revenue.services.statements import close_period_for_user, previous_calendar_month
    ...
    user_ids = (
        VideoJob.objects.filter(
            is_platform_generated=True, published_at__date__gte=start, published_at__date__lt=end
        )
    ...
    for user in User.objects.filter(id__in=user_ids):
        try:
            ...
            failed += 1
            logger.exception("revenue_period_close_user_failed", extra={"user_id": str(user.id)})
            continue
    ...
    result = {"period_start": start.isoformat(), "period_end": end.isoformat(), "closed": closed, "skipped": skipped, "failed": failed}
