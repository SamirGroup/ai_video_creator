...
import { mockAdSenseAccount, mockChannel } from '@/mocks/fixtures'
import { mockFetch } from '@/mocks/mockFetch'

// TODO: real API — replace with channelsApi.list()/getAdsenseAccount() (src/api/channels.ts)
function useChannel() {
  return useQuery({ queryKey: ['channel'], queryFn: () => mockFetch(mockChannel) })
}
function useAdsenseAccount() {
  return useQuery({ queryKey: ['adsense-account'], queryFn: () => mockFetch(mockAdSenseAccount) })
}
      ...
      {channelQuery.data === null && (
        <EmptyState
          ...
          action={
            ...
            <Button>{t('channel.connect')}</Button>
          }
        />
      )}
      ...
      {channelQuery.data && (
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0">
            ...
            <Badge tone={channelQuery.data.status === 'connected' ? 'success' : 'destructive'} dot>
          ...
          <CardContent className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              <div>
                <p className="text-xs text-muted-foreground">{t('channel.subscribers')}</p>
            ...
            <div className="border-t border-border pt-4">
              <p className="mb-2 text-sm font-medium text-foreground">{t('channel.adsense.title')}</p>
              {adsenseQuery.isLoading && <p className="text-sm text-muted-foreground">{t('common.loading')}</p>}
              {adsenseQuery.data ? (
            ...
            {showDisconnectConfirm ? (
              <div className="flex flex-col gap-2 rounded-md border border-destructive-600/40 bg-destructive-50 p-3 dark:bg-destructive-700/10">
                ...
                <div className="flex justify-end gap-2">
                  <Button size="sm" variant="ghost" onClick={() => setShowDisconnectConfirm(false)}>
                    {t('common.cancel')}
                  </Button>
              ...
              <div>
                <Button size="sm" variant="outline" onClick={() => setShowDisconnectConfirm(true)}>
                  {t('channel.disconnect')}
                </Button>
              </div>
            )}
